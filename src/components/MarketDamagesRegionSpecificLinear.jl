@defcomp MarketDamagesRegionSpecificLinear begin
    region = Index()
    ...
    impf_coeff_lin_regionspecificlinear = Parameter(index = [region], unit = "none") # regional damage functions based on temperature and growth projections
    ...
    # include error variance of region-specific damage function in Monte Carlo runs
    errvarlin_errorvarianceregionspecificlinear = Parameter(index = [region])
    ...
    function run_timestep(p, v, d, t)
        ...
        for r in d.region
            ...
            v.i1log_impactlogchange[t,r] = p.nlag_burke * (p.impf_coeff_lin_regionspecificlinear[r]  * (v.i_burke_regionalimpact[t,r] - p.rtl_abs_0_realizedabstemperature[r]) +
                                                            p.mcsignal_montecarlorunsignal * (rand(Normal(0, p.errvarlin_errorvarianceregionspecificlinear[r]^0.5), 1)[1] - # error variances are only included for Monte Carlo runs
                                                                 rand(Normal(0, p.errvarlin_errorvarianceregionspecificlinear[r]^0.5), 1)[1]))
...
function addmarketdamagesregionspecificlinear(model::Model)
    marketdamagesregionspecificlinearcomp = add_comp!(model, MarketDamagesRegionSpecificLinear)
    marketdamagesregionspecificlinearcomp[:rtl_abs_0_realizedabstemperature] = readpagedata(model, "data/rtl_abs_0_realizedabstemperature.csv")
    marketdamagesregionspecificlinearcomp[:rtl_0_realizedtemperature] = readpagedata(model, "data/rtl_0_realizedtemperature.csv")

    return marketdamagesregionspecificlinearcomp
end

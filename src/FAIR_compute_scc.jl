...
for usg in usg_scenario
    ...
    for pulse_year in pulse_years
        ...
        input_temp = temperature[[year in page_years for year in fair_years], :T] # global average temperature
        realized_temperature = input_temp * m[:ClimateTemperature, :ampf_amplification]' # regional realized temperatures

        ## set parameters in PAGE model
        ...
        new_realized_temperature = new_input_temp * m[:ClimateTemperature, :ampf_amplification]'
        ...
        scc = mm[:EquityWeighting, :td_totaldiscountedimpacts] / MimiPAGE2020.undiscount_scc(mm.base, pulse_year) * 1e6 / 1e9 # millions to dollars
        println(scc)
...
new_input_temp .- input_temp


PAGE_mm[:EquityWeighting, :td_totaldiscountedimpacts] / MimiPAGE2020.undiscount_scc(PAGE_m, pulse_year) # why isnt this the same as when you use compute_scc?
PAGE_mm.base[:EquityWeighting, :emuc_utilityconvexity]
PAGE_mm.base[:EquityWeighting, :ptp_timepreference]



MimiPAGE2020.compute_scc(year = 2030, pulse_size = 7500., prtp = 0.03, eta = 0.)
